...
## コンパクト化(Phase 3.5b — 承認済み追補)

初回実装(ラベル上置き・セル高 ≈36px・ボタン 44px)は縦密度が低く、最小
ウィンドウを更に縮めたいというステークホルダー要望により、**ラベル左置き
グリッド + アイコン優先**へ圧縮する。セクション順・機能は不変。

```
┌ gemelli ────────────────┐
│ DEVICE [FaceTime HD…▾][⟳]│  ← ラベル列は固定幅、行高 24px
│ ROTATE [ 0°│90°│180°│270°]│
│ FLIP   [ — │ ⇋ │ ⇅ │ ⇋⇅ ]│  ← グリフは実装時検証、無ければテキスト
│ CROP   [ off │ edit… ]    │
│ SCALE  [ off │ factor│W×H]│
│ SERVER [gemelli__________]│
│ [ ▶ START PUBLISHING ]    │  ← 28px、▶/■ + テキスト
├──────────────────────────┤
│         preview          │
├──────────────────────────┤
│ ○ stopped│gemelli│…│0 fps│
└──────────────────────────┘
```

- **ラベル列**: `group_label` を左列(固定幅、右揃えか左揃えは実装時に
  読みやすい方)へ。眉ラベルの見た目(uppercase / TEXT_SUBTLE / 小サイズ)は維持。
- **行高**: segmented セル・combo・TextEdit を 24px(Cannelloni `targetMin`)、
  アクションボタンを 28px に。
- **アイコン**: フォントグリフや SVG asset は使わず、**egui の Painter で
  直接描くベクターアイコン**とする(依存追加ゼロ・license 影響なし・任意
  DPI でシャープ・2px stroke の neo-brutalist 描画系と統一。SVG 案は
  resvg/usvg が MPL-2.0 で permissive-only ポリシーに反するため不採用 —
  ステークホルダー決定)。widgets.rs に `fn paint_icon_*(painter, rect,
  color)` 群を追加: FLIP = none(—)/ 左右反転 / 上下反転 / 両方、
  refresh(円弧 + 矢頭)、play(▶ 三角)/ stop(■ 矩形)。
  色だけでなく形で判別できること(WCAG 1.4.1 相当)。
- **最小/初期ウィンドウ**: 控除部の実測に合わせて再計算する。
  - 最小: プレビュー高 ≥120px を確保できる最小値(目安 300×480 前後)。
  - 初期: **初期高もできるだけ小さく**する — 初期幅でプレビューが 16:9 で
    ちょうど収まる高さ(控除部 + statusbar + 幅×9/16)とし、余白を持たせない
    (目安 360×450 前後 — 実装時に実測で確定)。
- 24px は Cannelloni の `targetMin` そのもの(comfortable 44px ではない)である
  ことを認識のうえ、デスクトップツールとして採用する。
