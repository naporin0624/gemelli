...
- **行高**: segmented セル・combo・TextEdit を 24px(Cannelloni `targetMin`)、
  アクションボタンを 28px に。
- **アイコン**: FLIP は — / ⇋ / ⇅ / ⇋⇅ 相当、refresh ⟳、START/STOP は ▶/■ +
  テキスト。**egui のフォント(LINE Seed JP + egui 組み込み)に実グリフが
  あるか実装時に検証**し、tofu になるものはテキスト(H / V / H+V)へ
  フォールバック。色だけでなく形で判別できること(WCAG 1.4.1 相当)。
- **最小/初期ウィンドウ**: 控除部の実測に合わせて再計算し、最小はプレビュー
  高 ≥120px を確保できる値まで縮める(目安: 最小 300×480 前後、初期 360×640
  前後 — 実装時に実測で確定)。
